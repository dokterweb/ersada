...
class DashboardController extends Controller
{
    ...
    public function indexadmincabang()
    {
        ...
        $query = Pengajuan::where('cabang_id', $karyawan->cabang_id);

        $draft = (clone $query)
            ->where('status', 'draft')
            ->count();

        $menungguPimpinan = (clone $query)
            ->where('status', 'menunggu_pimpinan')
            ->count();

        $revisi = (clone $query)
            ->whereIn('status', [
                'revisi_marketing',
                'revisi_spv_marketing',
            ])
            ->count();

        $disetujui = (clone $query)
            ->where('status', 'disetujui_direktur')
            ->count();
        ...
        return view('dashboard.admincabang', compact(
            'draft',
            'menungguPimpinan',
            'revisi',
            'disetujui',
