...
class SurveyController extends Controller
{
        ...
        DB::transaction(function () use ($survey,$validated,$request){
            ...
            SurveyBerkas::updateOrCreate(
                ...
                [
                    ...
                    'ktp_pasangan'          =>$request->boolean('ktp_pasangan'),
                    'kk_pasangan'           =>$request->boolean('kk_pasangan'),
                    'status_peminjam'       =>$validated['status_peminjam'],
                    'plafond_pinjaman_lama' =>$validated['plafond_pinjaman_lama'],
                    'ktp_penjamin'          =>$request->boolean('ktp_penjamin'),
                    'kk_penjamin'           =>$request->boolean('kk_penjamin'),
                    'nama_pasangan_penjamin'=>$validated['nama_pasangan_penjamin'],
                    ...
                    'no_id_karyawan'        =>$validated['no_id_karyawan'],
                    'lama_bekerja'          =>$validated['lama_bekerja'],
                    'no_telp_karyawan'      =>$validated['no_telp_karyawan'],
                    ...
                    'pin_atm'               =>$validated['pin_atm'],
                    'catatan_kekurangan'    =>$validated['catatan_kekurangan'],
                ]
            );
        });
    ...
    public function review(Survey $survey)
    {
        ...
        if ($user->hasRole('surveyor')) {
            ...
            if ($survey->assigned_to == $user->id) {
                abort_if(!in_array($survey->status, ['waiting','accepted','progress','revision']),403);
                $mode = 'surveyor';
            } else {
                // SPV hanya melihat hasil survey bawahan
        ...
        $survey->load(['pengajuan','pengajuan.nasabah','pengajuan.nasabah.pekerjaan','pengajuan.marketing','pengajuan.cabang','pengajuan.jaminans',
            'pengajuan.referensis','berkas','dokumentasis','assignedTo']);
    ...
    public function submit(Request $request, Survey $survey)
    {
        ...
        $request->validate(['confirm_submit'=>'accepted']);
        ...
        return redirect()->route('survey.index')->with('success','Survey berhasil dikirim ke SPV Survey.');
    }
